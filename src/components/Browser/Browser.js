  ...
  render () {
    ...
    itemsJsx = items.map(item => (
      <Card key={item._id}
        ...
        border="primary"
        className='index-bg style-card' style={{ alignItems: 'center', borderRadius: '5px', boxShadow: '-1px 3px #919191', display: 'flex', height: '350px', margin: '40px', padding: '8px', marginTop: '10px' }}>
        <Card.Body className="card-b-style">
          <Card.Title className="card-title-font" style={{ alignSelf: 'center' }}>{item.name}</Card.Title>
    ...
    return (
      <div style={{ alignContent: 'center', display: 'flex', flexDirection: 'column' }}>
        <div className='happy-shopping-header'style={{ alignItems: 'center', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          <h2 className="browser-top-line">Happy Shopping!</h2>
