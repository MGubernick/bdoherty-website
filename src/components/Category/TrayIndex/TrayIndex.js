...
class TrayIndex extends Component {
  constructor (props) {
    super(props)
  ...
  componentDidMount () {
    ...
    indexItems()
      ...
      .catch(error => {
        msgAlert({
          message: `Couldn't load trays because: ${error.message}`,
          variant: 'danger'
        })
      })
  }
  ...
  render () {
    ...
    const trayItems = items.filter(item => item.category === 'Tray')

    if (trayItems.length === 0) {
      return (
        <div style={{ alignItems: 'center', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          <h6 className="home-page-topline">There are currently no trays available! More will be posted soon!</h6>
          <img style={{ width: '20%' }} src="https://media.giphy.com/media/3o85xILm2U2hVNhP2g/giphy.gif" alt="loading gif" />
        </div>
      )
    }

    const itemsJsx = trayItems.map(item => (
      <Card key={item._id}
        onClick={(event) => {
    ...
    return (
      <div style={{ alignContent: 'center', display: 'flex', flexDirection: 'column' }}>
        <div style={{ alignItems: 'center', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          <h2 className="home-page-topline" style={{ fontSize: '60px' }}>Trays!</h2>
...
export default withRouter(TrayIndex)
